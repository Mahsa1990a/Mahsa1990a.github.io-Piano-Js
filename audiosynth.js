...
!function(){
	...
	let setPrivatelet = function(n,v,w,e){Object.defineProperty(this,n,{value:v,writable:!!w,enumerable:!!e});};
	let setPubliclet = function(n,v,w){setPrivatelet.call(this,n,v,w,true);};
	AudioSynthInstrument = function AudioSynthInstrument(){this.__init__.apply(this,arguments);};
	let setPriv = setPrivatelet.bind(AudioSynthInstrument.prototype);
	let setPub = setPubliclet.bind(AudioSynthInstrument.prototype);
	setPriv('__init__', function(a,b,c) {
		...
		setPrivatelet.call(this, '_parent', a);
		setPubliclet.call(this, 'name', b);
		setPrivatelet.call(this, '_soundID', c);
	});
	...
	setPriv = setPrivatelet.bind(AudioSynth.prototype);
	setPub = setPubliclet.bind(AudioSynth.prototype);
	...
	setPub('generate', function(sound, note, octave, duration) {
		...
		if(typeof(this._fileCache[sound][octave-1][note][time])!='undefined') {
			...
			let waveBind = {modulate: this._mod, lets: this._temp};
			let val = 0;
			let curVol = 0;
...
},
{
	...
	wave: function(i, sampleRate, frequency) {
		...
		if(typeof(lets.playVal)=='undefined') { lets.playVal = 0; }
		if(typeof(lets.periodCount)=='undefined') { lets.periodCount = 0; }
	
		let valueTable = lets.valueTable;
		let playVal = lets.playVal;
		let periodCount = lets.periodCount;
		...
		if(valueTable.length<=Math.ceil(period)) {
			...
			if(resetPlay) { lets.playVal = 0; } else { lets.playVal++; }
